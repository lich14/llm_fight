...
class GameAPI:
    ...
    def _handle_response(self, response: dict, error_msg: str) -> Any:
        """处理API响应，提取所需数据或抛出异常"""
        if response is None:
            raise GameAPIError("NO_RESPONSE", "{0}".format(error_msg))
    ...
    def can_produce(self, unit_type: str) -> bool:
        ...
        try:
            ...
            raise GameAPIError("PRODUCE_QUERY_ERROR",
                               "查询生产能力时发生错误: {0}".format(str(e)))

    def produce(self,
                unit_type: str,
                quantity: int,
                auto_place_building: bool = False) -> Optional[int]:
        '''生产指定数量的Actor
        ...
        try:
            ...
            raise GameAPIError("PRODUCTION_ERROR",
                               "执行生产命令时发生错误: {0}".format(str(e)))

    def produce_wait(self,
                     unit_type: str,
                     quantity: int,
                     auto_place_building: bool = True) -> None:
        '''生产指定数量的Actor并等待生产完成
        ...
        try:
            ...
            raise GameAPIError("PRODUCTION_WAIT_ERROR",
                               "生产并等待过程中发生错误: {0}".format(str(e)))
    ...
    def is_ready(self, wait_id: int) -> bool:
        ...
        '''
        try:
            response = self._send_request('query_wait_info',
                                          {"waitId": wait_id})
            ...
            raise GameAPIError("WAIT_STATUS_ERROR",
                               "查询任务状态时发生错误: {0}".format(str(e)))
    ...
    def move_units_by_location(self,
                               actors: List[Actor],
                               location: Location,
                               attack_move: bool = False) -> None:
        '''移动单位到指定位置
        ...
        try:
            ...
            raise GameAPIError("MOVE_UNITS_ERROR",
                               "移动单位时发生错误: {0}".format(str(e)))

    def move_units_by_location_and_id(self,
                                      ids,
                                      location: Location,
                                      attack_move: bool = False) -> None:
        '''移动单位到指定位置
        ...
        try:
            response = self._send_request(
                'move_actor', {
                    "targets": {
                        "actorId": [actor_id for actor_id in ids]
                    },
                    "location": location,
                    "isAttackMove": 1 if attack_move else 0
                })
    ...
    def move_units_by_direction(self, actors: List[Actor], direction: str,
                                distance: int) -> None:
        '''向指定方向移动单位
    ...
    def move_units_by_path(self, actors: List[Actor],
    ...
    def select_units(self, query_params: TargetsQueryParam) -> None:
        ...
        try:
            ...
            raise GameAPIError("SELECT_UNITS_ERROR",
                               "选择单位时发生错误: {0}".format(str(e)))
    ...
    def form_group(self, actors: List[Actor], group_id: int) -> None:
        ...
        try:
            ...
            raise GameAPIError("FORM_GROUP_ERROR",
                               "编组时发生错误: {0}".format(str(e)))
    ...
    def query_actor(self, query_params: TargetsQueryParam) -> List[Actor]:
        ...
        try:
            ...
            for data in actors_data:
                try:
                    actor = Actor(data["id"])
                    position = Location(data["position"]["x"],
                                        data["position"]["y"])
                    hp_percent = data["hp"] * 100 // data["maxHp"] if data[
                        "maxHp"] > 0 else -1
                    actor.update_details(data["type"], data["faction"],
                                         position, hp_percent)
                    actors.append(actor)
                except KeyError as e:
                    raise GameAPIError("INVALID_ACTOR_DATA",
                                       "Actor数据格式无效: {0}".format(str(e)))
            ...
            raise GameAPIError("QUERY_ACTOR_ERROR",
                               "查询Actor时发生错误: {0}".format(str(e)))

    def find_path(self, actors: List[Actor], destination: Location,
                  method: str) -> List[Location]:
        '''为Actor找到到目标的路径
        ...
        try:
            ...
            try:
                return [
                    Location(step["x"], step["y"]) for step in result["path"]
                ]
            except (KeyError, TypeError) as e:
                raise GameAPIError("INVALID_PATH_DATA",
                                   "路径数据格式无效: {0}".format(str(e)))

        except GameAPIError:
            raise
        except Exception as e:
            raise GameAPIError("FIND_PATH_ERROR",
                               "寻路时发生错误: {0}".format(str(e)))
    ...
    def get_actor_by_id(self, actor_id: int) -> Optional[Actor]:
        ...
        try:
            ...
            raise GameAPIError("GET_ACTOR_ERROR",
                               "获取Actor时发生错误: {0}".format(str(e)))
    ...
    def update_actor(self, actor: Actor) -> bool:
        ...
        try:
            ...
            try:
                ...
                position = Location(actor_data["position"]["x"],
                                    actor_data["position"]["y"])
                hp_percent = actor_data["hp"] * 100 // actor_data[
                    "maxHp"] if actor_data["maxHp"] > 0 else -1
                actor.update_details(actor_data["type"], actor_data["faction"],
                                     position, hp_percent)
                return True
            ...
            raise GameAPIError("UPDATE_ACTOR_ERROR",
                               "更新Actor信息时发生错误: {0}".format(str(e)))
    ...
    def deploy_units(self, actors: List[Actor]) -> None:
        ...
        try:
            ...
            raise GameAPIError("DEPLOY_UNITS_ERROR",
                               "部署单位时发生错误: {0}".format(str(e)))
    ...
    def occupy_units(self, occupiers: List[Actor],
        ...
        try:
            response = self._send_request(
                'occupy', {
                    "occupiers": {
                        "actorId": [actor.actor_id for actor in occupiers]
                    },
                    "targets": {
    ...
    def attack_target_id(self, attacker_id, target_id_list) -> bool:
        '''攻击指定目标
        ...
        try:
            ...
            raise GameAPIError("ATTACK_ERROR",
                               "攻击命令执行时发生错误: {0}".format(str(e)))
    ...
    def can_attack_target(self, attacker: Actor, target: Actor) -> bool:
        ...
        try:
            ...
            raise GameAPIError("CHECK_ATTACK_ERROR",
                               "检查攻击能力时发生错误: {0}".format(str(e)))
    ...
    def repair_units(self, actors: List[Actor]) -> None:
        ...
        try:
            ...
            raise GameAPIError("REPAIR_ERROR",
                               "修复命令执行时发生错误: {0}".format(str(e)))
    ...
    def visible_query(self, location: Location) -> bool:
        ...
        try:
            ...
            raise GameAPIError("VISIBILITY_QUERY_ERROR",
                               "查询可见性时发生错误: {0}".format(str(e)))
    ...
    def explorer_query(self, location: Location) -> bool:
        ...
        try:
            ...
            raise GameAPIError("EXPLORER_QUERY_ERROR",
                               "查询探索状态时发生错误: {0}".format(str(e)))
    ...
    def query_production_queue(self, queue_type: str) -> dict:
        ...
        if queue_type not in [
                'Building', 'Defense', 'Infantry', 'Vehicle', 'Aircraft',
                'Naval'
        ]:
            raise GameAPIError(
                "INVALID_QUEUE_TYPE",
        ...
        try:
            ...
            raise GameAPIError("PRODUCTION_QUEUE_QUERY_ERROR",
                               "查询生产队列时发生错误: {0}".format(str(e)))
    ...
    def place_building(self,
        ...
        try:
            ...
            raise GameAPIError("PLACE_BUILDING_ERROR",
                               "放置建筑时发生错误: {0}".format(str(e)))
    ...
    def manage_production(self, queue_type: str, action: str) -> None:
        ...
        if action not in ['pause', 'cancel', 'resume']:
            raise GameAPIError("INVALID_ACTION",
                               "action参数必须是 'pause', 'cancel', 或 'resume'")

        try:
            params = {"queueType": queue_type, "action": action}
            ...
            raise GameAPIError("MANAGE_PRODUCTION_ERROR",
                               "管理生产队列时发生错误: {0}".format(str(e)))
    ...
    def get_unexplored_nearby_positions(self, map_query_result: MapQueryResult,
    ...
    def move_units_by_location_and_wait(self,
                                        actors: List[Actor],
                                        location: Location,
                                        max_wait_time: float = 10.0,
                                        tolerance_dis: int = 1) -> bool:
    ...
    def unit_attribute_query(self, actors: List[Actor]) -> dict:
        ...
        try:
            ...
            raise GameAPIError("ATTRIBUTE_QUERY_ERROR",
                               "查询Actor属性时发生错误: {0}".format(str(e)))
    ...
    def map_query(self) -> MapQueryResult:
        ...
        try:
            ...
            return MapQueryResult(MapWidth=result.get('MapWidth', 0),
                                  ...
                                  ResourcesType=result.get(
                                      'ResourcesType', [[]]),
                                  Resources=result.get('Resources', [[]]))
        except GameAPIError:
            raise
        except Exception as e:
            raise GameAPIError("MAP_QUERY_ERROR",
                               "查询地图信息时发生错误: {0}".format(str(e)))
    ...
    def player_base_info_query(self) -> PlayerBaseInfo:
        ...
        try:
            ...
            return PlayerBaseInfo(Cash=result.get('Cash', 0),
            ...
            raise GameAPIError("BASE_INFO_QUERY_ERROR",
                               "查询玩家基地信息时发生错误: {0}".format(str(e)))
    ...
    def screen_info_query(self) -> ScreenInfoResult:
        ...
        try:
            ...
            return ScreenInfoResult(
                ScreenMin=Location(result['ScreenMin']['X'],
                                   result['ScreenMin']['Y']),
                ScreenMax=Location(result['ScreenMax']['X'],
                                   result['ScreenMax']['Y']),
                IsMouseOnScreen=result.get('IsMouseOnScreen', False),
                MousePosition=Location(result['MousePosition']['X'],
                                       result['MousePosition']['Y']))
        except GameAPIError:
            raise
        except Exception as e:
            raise GameAPIError("SCREEN_INFO_QUERY_ERROR",
                               "查询屏幕信息时发生错误: {0}".format(str(e)))

    def set_rally_point(self, actors: list[Actor],
        ...
        try:
            ...
            raise GameAPIError("SET_RALLY_POINT_ERROR",
                               "设置集结点时发生错误: {0}".format(str(e)))
